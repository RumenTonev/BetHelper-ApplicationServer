...
namespace BetHelper.Web.Models.Players
{
   ...
   public PlayerShortModel(PlayersNewEntryPushModel model)
        { }
        public PlayerShortModel(string teamName,Player player)
        {
            ...
            int todayDay = DateTime.Now.Day;
                   ...
                   this.Activities = dat.Activities.All().Where(x => x.PlayerId == player.PlayerId).Where(x => x.Profile.TeamName == teamName
                      ...
                      && x.Profile.Game.GameStatus != GameStatus.PostPoned).OrderByDescending(y => y.Profile.Game.GameDate).Take(10).OrderBy(x => x.Profile.Game.GameDate).AsQueryable().Include(x => x.RedCard).ToList()
